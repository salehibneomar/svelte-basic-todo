...
<div class="mx-auto mt-10 max-w-xl rounded-lg bg-white p-8 shadow-lg">
	...
	<form class="mb-6 flex gap-2">
		<input
			...
			class="flex-1 rounded border border-slate-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-slate-400"
		/>
		<button
			type="submit"
		...
		{#each $todos as todo}
			...
			>
				<div class="flex items-center gap-2">
					<input type="checkbox" class="ml-2 accent-slate-700" checked={Boolean(todo.is_completed)} />
					<span class={Boolean(todo.is_completed) ? 'text-slate-400 line-through' : 'text-slate-700'}>{todo.title}</span>
				</div>
	...
	<nav class="mt-8 flex justify-center" aria-label="Pagination">
		<ul class="inline-flex items-center -space-x-px text-sm">
			<li>
				<button
					type="button"
					class="ms-0 flex h-8 items-center justify-center rounded-s border border-e-0 border-slate-300 bg-white px-2 leading-tight text-slate-500 hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-400 transition-colors"
				>
					Previous
				</button>
			</li>
			...
			{#each Array(pagingData.last_page) as _, index}
				<li>
					<button
						type="button"
						class="flex h-8 items-center justify-center border border-slate-300 px-2 leading-tight transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400 cursor-pointer
			...
			<li>
				<button
					type="button"
					class="flex h-8 items-center justify-center rounded-e border border-slate-300 bg-white px-2 leading-tight text-slate-500 hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-400 transition-colors"
				>
					Next
				</button>
			</li>
